...
class Exception_handling{
	...
	public:
		...
		static bool checkcontact(const char cont[9]){												//function to check whether the cell number contains any invalid char or not
			int flag = 0, cnt = 0;
...
class Cashier : public Employees{
	...
	public:
		...
		void setcustom_ptr(Customer &Cust){ //Cashier
			C = &Cust;
		}
		...
		bool checkitem(const long ID){	//calls check_availability for each item from inventory	and call another func to add it's price if available
			float price;
		...
		Cashier(char nme[50], char add[100], char Email[50], long long Mobile){										//parametrised cstr
			...
			cout<< endl<< "Enter a number of 3 UNEQUAL digits to add it in the password (each digit should be Greater than 0):"<< endl;
			do{
				cin>> random;
				n = random;
				cnt = 2;
		...
		//billings:
		void finaliseBill(){		//When the price of all the items will be added to the bill, then to add it to the record file
			C->AddRecord();
			customers+= 1;
			updaterecord();
		}
...
class Manager : public Employees{
	...
	public:
		...
		Manager() : identity(999){
			setidm(identity);
			setemail("200602942@students.nust.na");
			setpswd("Mervin0123");
			setaddress("Hockland, Windhoek");
		}
		...
		void display_salesperday(){	//function to display the amount for the sales of each day
			ifstream Customers;
	...
	while(1){
		...
		while(1){
			...
			if(choice == '0' || choice == '1')
				...
				char a;
				cout << endl<< endl<< endl<< "Process exited with exception(s) value "<< E.getexception(); //here to keep the total values ​​of exception handling (after)
				a = getch();
				exit(1);
			}
		...
		if(choice == '0'){ 						// Cashier's Module
			...
			do{
				...
				if(strcmp(menu, "1") == 0){
					...
					In.display_specificprod(id);
					cout<< endl<< endl;
					In.clritem_ptr();	//If there is an error, then it is attached here.
				}
